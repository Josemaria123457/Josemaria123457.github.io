...
export default function PageServicios() {
  const ld = {
    ...
    address: { "@type": "PostalAddress", addressLocality: "Oaxaca", addressCountry: "MX" },
    knowsAbout: ["landing pages", "sitios corporativos", "e-commerce", "SEO técnico", "web performance"],
    ...
    <>
      ...
      <main className={styles.container}>
        ...
        <section className={styles.hero} id="top">
          <p className={styles.eyebrow}>
            <span className={styles.badge}>Web performance • SEO • Conversión</span>
          </p>
          <h1 className={styles.heroTitle}>
            Servicios de diseño y desarrollo web orientados a <span className={styles.accent}>resultados</span>
          </h1>
          <p className={styles.heroSubtitle}>
            Paquetes flexibles para negocios locales, emprendedores y pymes.
            Entregas rápidas, SEO técnico, analítica y soporte con enfoque en rendimiento y conversión.
          </p>
          ...
          <div className={styles.highlights} role="list" aria-label="atributos">
            ...
            <div className={styles.highlight} role="listitem"><strong>Soporte</strong><span>Acompañamiento post-lanzamiento</span></div>
          </div>
        </section>
        ...
        <section className={styles.section} id="contacto" aria-labelledby="t-contacto">
          <div className={styles.ctaBox}>
            ...
            <div className={styles.ctaActions}>
              ...
              <a className={styles.btn} href="mailto:hola@pinonet.me" data-cta="email">Enviar correo</a>
            </div>
          </div>
        </section>
      </main>
      ...
      <footer className={styles.footer}>
        <div className={styles.container}>
          © {new Date().getFullYear()} PINONET • Hecho con ♥ • Rendimiento • SSL • Deploy en Vercel
        </div>
      </footer>
    </>
  );
...
function ServiceCard(props: {
  ...
  const {
    eyebrowIcon, eyebrow, title, desc, features, price,
    primaryHref, primaryLabel = "Solicitar cotización",
    ...
    <article className={styles.card}>
      <div className={styles.cardBody}>
        ...
        <ul className={styles.features}>
          {features.map((f, i) => (
            <li key={i} className={styles.feature}><Check /> {f}</li>
          ))}
        </ul>
        <div className={styles.price}>
          <strong>{price}</strong>{price.includes("MXN") && <span className={styles.muted}> (desde)</span>}
        </div>
...
function CartIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M3 5h18l-2 10H5L3 5Zm4 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm10 0a2 2 0 1 0 0-4 2 2 0 0 0 0 4Z" stroke="currentColor" strokeWidth="2"/>
    </svg>
  );
}
function LayoutIcon() {
